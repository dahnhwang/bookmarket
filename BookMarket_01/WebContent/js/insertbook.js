	...
	$
			.ajax({
				...
				data : {
					...
					condition : parseInt($('#stars li.selected').last().data(
							'value'), 10),
					price_type : $('input[name=price_type]:checked',
							'#price_frm').val(),
					price : $('input[name=price]', '#price_input').val(),
					due_date : (typeof $('input[type=date]', '#price_date')
							.val() != "undefined") ? $('input[type=date]',
							'#price_date').val() : "1999-01-31"
				},
